...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "arkive-operator-lock.backup.arkive.io",
	})
	if err != nil {
	...
	// Initialize Storage Client (uses FakeStorageClient for dev/testing when AWS credentials absent)
	storageClient := storage.NewFakeStorageClient()
	...
	if err = (&controllers.DatabaseRestoreReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	setupLog.Info("starting Arkive operator manager")
